import type { InterfaceDataModel, InterfaceDataWrapper } from "../types";
 ...
 * ```
 */
export class DataWrapper implements InterfaceDataWrapper {
    /**
    ...
    clone = (): InterfaceDataWrapper | null => {
        ...
        for (let key in this._data) {
            newData[key] = hisonCore.utils.deepCopyObject(this._data[key]);
        }
        return new hisonCore.data.DataWrapper(newData);
    };
    /**
    ...
    values = (): InterfaceDataModel<any>[] | string[] => {
        ...
        for (let key in this._data) {
            if (this._data.hasOwnProperty(key)) {
                values.push(hisonCore.utils.deepCopyObject(this._data[key]));
            }
        }
        return values;
    };
