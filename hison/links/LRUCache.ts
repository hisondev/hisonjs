import { Hison } from "../types";

export class LRUCache {
    constructor(hison: Hison, limit: number) {
        this._hison = hison;
        this._limit = limit;
        this._cache = {};
        this._keys = [];
    }
    private _hison: Hison;
    ...
    get = (key: string): Promise<{ data: any; response: Response; }> | null => {
        ...
        const value = this._hison.utils.deepCopyObject(this._cache[key]);
        this._removeKey(key);
        this._keys.push(key);
        return value;
    };
    put = (key: string , value: Promise<{ data: any; response: Response; }>) => {
        ...
        this._cache[key] = this._hison.utils.deepCopyObject(value);
        this._keys.push(key);
    };
    remove = (key: string): Promise<{ data: any; response: Response; }> | null => {
        if(!this.hasKey(key)) return null;
        this._removeKey(key);
        const result = this._hison.utils.deepCopyObject(this._cache[key])
    ...
    getAll = (): Record<string, Promise<{ data: any; response: Response; }>> => {
        ...
        Object.keys(this._cache).forEach((key) => {
            result[key] = this._hison.utils.deepCopyObject(this._cache[key]);
        });
        return result;
    };
    getKeys = (): string[] => {
